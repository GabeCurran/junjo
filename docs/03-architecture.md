...
- `events.ts` - construction-and-publish helpers shared by every mutation route. Exports `newEventId()` (24-char hex from `node:crypto`, used to stamp `event.id`), `publishEvent<E extends JunjoEvent>(hub, payload)` (auto-stamps `id` + `occurredAt`, calls `hub.publish`, returns the constructed event), and a family of brand-cast converters (`toPublicGroup`, `toPublicMember`, `toPublicRole`, `toPublicInvitation`, `toPublicGroupRelationship`) that turn a Prisma row into the public `@junjo/shared` shape that the `JunjoEvent` payloads expect. The converters are pure brand-casts plus the `memberCount` / `roleIds` / `permissions` join columns that the public shapes carry; no field-name remapping happens.
...
- **Webhooks** (Phase 5.3): persistent fan-out to dev-configured HTTP endpoints. Backed by `WebhookDelivery` rows so retries and at-least-once semantics are durable across process restarts.
...
- Mutation routes call `hub.publish(event)` after their transaction commits (the same after-commit pattern used by `permissionCache.invalidateGroup`). The construction helper lives in `events.ts`: routes call `publishEvent<E>(hub, payload)`, which stamps `id` and `occurredAt` and invokes the hub. The hub is threaded into each mutation router via `createApp({ events: { hub } })` so tests can swap in a fresh `EventHub` without touching the singleton; production uses the module-level singleton. A crash between commit and publish loses the event for transient subscribers; webhooks (Phase 5.3) and `audit.list` (Phase 5.2) provide the durable counterparts.
