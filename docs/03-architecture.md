...
Phase 11.1a + 11.1b (the toolchain + visible-shell foundation) plus Phase 11.2a + 11.2b (cross-game admin endpoints + the home page that consumes them) plus Phase 11.3a + 11.3b-i (cross-game games + API key management endpoints, plus the games list page consuming them) ship:
...
- **Games list page (Phase 11.3b-i):** `app/(dashboard)/games/page.tsx` composes `<CreateGameDialog>` (in the `<Topbar>`'s `actions` slot) plus a `<Suspense>`-wrapped `<GamesList>` Server Component. `GamesList` (`components/dashboard/games-list.tsx`) calls `fetchAdminGames()` and renders the rows in a plain HTML `<table>` with five columns (Name + game id, Groups count, Active members count, API keys count, Created date) plus an "Open" link to `/games/[gameId]` (Phase 11.3b-ii). The same three failure-mode shells as the home page (admin-disabled / generic-error / empty) ship per-component. `CreateGameDialog` (`components/dashboard/create-game-dialog.tsx`) is a Client Component that wraps a `<form>` whose `action` prop is `useFormState(createGameAction, INITIAL_STATE)`. `createGameAction` (in `app/(dashboard)/games/actions.ts`, marked `"use server"`) validates the form's `name` field via Zod (mirrors `createGameBody` 1-200 chars), calls `createAdminGame`, and calls `revalidatePath("/games")` before returning a typed `CreateGameResult`. The dialog reads the result via `useFormState`, closes on success, surfaces validation errors inline on failure, and shows a pending "Creating..." submit-button label via `useFormStatus`.

What 11.3b-ii adds: the game detail page (`(dashboard)/games/[gameId]/page.tsx`) plus the API keys section with show-once-on-create dialog and the `createApiKeyAction` / `revokeApiKeyAction` Server Actions. What 11.4 - 11.9 add: the per-group data-bound pages (group browser, group detail tabs, game-wide audit, permission tester). What 12.1 - 12.5 add: the analytics surface.
