...
- **shadcn primitives:** populated by hand-vendoring the canonical shadcn registry source into `components/ui/` rather than running the interactive shadcn CLI. Phase 11.1b shipped `button.tsx`; Phase 11.2b shipped `card.tsx` (Card / CardHeader / CardTitle / CardDescription / CardContent / CardFooter, the standard shadcn container family); Phase 11.3b-i shipped `dialog.tsx` (Dialog / DialogTrigger / DialogContent / DialogHeader / DialogFooter / DialogTitle / DialogDescription / DialogClose / DialogOverlay / DialogPortal, wrapping `@radix-ui/react-dialog`), `input.tsx` (the canonical text-input primitive), and `label.tsx` (wrapping `@radix-ui/react-label`); Phase 11.3b-ii shipped `badge.tsx` (a `cva`-styled inline label primitive used to render the Active / Revoked status pill on each API key row). Future iterations add primitives to the same directory as features land that need them. The vendored source carries the proprietary license header but is otherwise byte-identical to the upstream registry, so `git diff`-against-upstream merges of future fixes stay cheap.
- **Admin API client:** `apps/dashboard/lib/admin.ts` is a `import "server-only"`-guarded module exporting GET helpers (`fetchAdminStats`, `fetchRecentAudit`, `fetchAdminGames`, `fetchAdminGame`, `fetchAdminApiKeys`, `fetchAdminGroupsForGame`) plus mutation helpers (`createAdminGame`, `createAdminApiKey`, `revokeAdminApiKey`) plus the typed wire shapes (`AdminStats`, `AdminAuditEntry`, `AdminAuditPage`, `AdminGame`, `AdminGameList`, `AdminApiKey`, `AdminApiKeyList`, `AdminApiKeyCreated`, `AdminGroup`, `AdminGroupList`, plus the `AdminGroupVisibility` / `AdminGroupSort` / `AdminGroupOrder` string-union types and the `ADMIN_GROUPS_PAGE_SIZE_OPTIONS` / `ADMIN_GROUPS_DEFAULT_PAGE_SIZE` constants the group browser surfaces in its page-size selector). All functions read `getAdminToken()` from the SDK singleton's helper; if the token is unset the function throws `AdminDisabledError` (a sentinel class the consumer branches on to render a "set `JUNJO_ADMIN_TOKEN` to enable this view" empty state instead of a generic crash). GET helpers pass `next: { revalidate: 60 }` so Next.js caches the response for 60 seconds; mutation helpers pass `cache: "no-store"`. The wire types mirror `WireAdminStats` / `WireAdminAuditEntry` / `WireAdminGame` / `WireAdminApiKey` etc. from `packages/server/src/routes/admin.ts` but live in the dashboard repo because the admin endpoints are intentionally not in the per-game `@junjo/sdk` surface (per Phase 10.2 + 11.2a + 11.3a decisions).
...
What 11.4 - 11.9 add: the per-group data-bound pages (group browser, group detail tabs, game-wide audit, permission tester). What 12.1 - 12.5 add: the analytics surface.
