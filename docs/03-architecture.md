...
- `routes/admin.ts` - cloud-only admin route handlers (Phase 10.2 + Phase 11.2a; marked `// @cloud-only`). Exports the `WireUserGames` / `WireUserGameRow` types and `listUserGamesHandler(prisma)` for `GET /v1/users/:junjoUserId/games`. The handler queries every `ExternalIdentity` row for the supplied internal `JunjoUser.id`, then runs a single batched `groupMember.findMany` to count active memberships in non-soft-deleted groups, tallying in memory (no N+1 counts). A `junjoUserId` with no identity rows returns `200` with `games: []` rather than 404 - the route cannot distinguish "user we have never seen" from "user known but no cross-game footprint yet" and collapses the cases. Games where the user has an identity but no active memberships are still listed with `joinedGroupCount: 0`. Phase 11.2a extends the file with two more handlers consumed by the dashboard home page: `getAdminStatsHandler(prisma)` (`GET /v1/admin/stats`) returns aggregate `{ totalGames, totalGroups, totalActiveMembers, totalAuditEntriesLast24h }` via four parallel `prisma.count` queries; `listRecentAuditHandler(prisma)` (`GET /v1/admin/audit?limit=20`) returns the most recent audit entries across every game on the deployment, with `gameName` / `groupName` / `groupSoftDeleted` pivoted into each item via a single `include`. All three routes are registered in `app.ts` BEFORE the per-game `apiKeyMiddleware` (same pattern as the public `getInvitationByCodeHandler`), with a per-route `adminAuthMiddleware` so the admin token is the only auth check that runs. Co-located Zod schema in `routes/admin.schema.ts` validates the audit endpoint's `?limit=` query; the user-games handler takes a path param only and needs no schema. Counting rules across stats: `totalGroups` and `totalActiveMembers` exclude soft-deleted groups (active-set semantics, mirrors `Group.memberCount` and the permission resolver), but `totalAuditEntriesLast24h` and the audit feed itself include soft-deleted-group entries because the audit log preserves history regardless of lifecycle state.
