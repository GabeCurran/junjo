...
Phase 11.1a + 11.1b (the toolchain + visible-shell foundation) plus Phase 11.2a + 11.2b (cross-game admin endpoints + the home page that consumes them) plus Phase 11.3a + 11.3b-i + 11.3b-ii (cross-game games + API key management endpoints, plus the games list and per-game detail pages consuming them; Phase 11.3 closes here) ship:

- **Stack:** Next.js 15 App Router, TypeScript strict, React 18, Tailwind v3 with shadcn-compatible CSS-variable theming, zod (env validation), `clsx` + `tailwind-merge` (shadcn-style class composition), `next-themes` (theme switching), `lucide-react` (icons), `class-variance-authority` (shadcn variant API), `@radix-ui/react-slot` (Button `asChild`), `@radix-ui/react-dialog` (Dialog primitive), `@radix-ui/react-label` (Label primitive), `tailwindcss-animate` (the canonical shadcn animation companion plugin powering the dialog's `data-[state=open]:animate-in` classes).
...
- **Admin API client:** `apps/dashboard/lib/admin.ts` is a `import "server-only"`-guarded module exporting GET helpers (`fetchAdminStats`, `fetchRecentAudit`, `fetchAdminGames`, `fetchAdminGame`, `fetchAdminApiKeys`) plus mutation helpers (`createAdminGame`, `createAdminApiKey`, `revokeAdminApiKey`) plus the typed wire shapes (`AdminStats`, `AdminAuditEntry`, `AdminAuditPage`, `AdminGame`, `AdminGameList`, `AdminApiKey`, `AdminApiKeyList`, `AdminApiKeyCreated`). All functions read `getAdminToken()` from the SDK singleton's helper; if the token is unset the function throws `AdminDisabledError` (a sentinel class the consumer branches on to render a "set `JUNJO_ADMIN_TOKEN` to enable this view" empty state instead of a generic crash). GET helpers pass `next: { revalidate: 60 }` so Next.js caches the response for 60 seconds; mutation helpers pass `cache: "no-store"`. The wire types mirror `WireAdminStats` / `WireAdminAuditEntry` / `WireAdminGame` / `WireAdminApiKey` etc. from `packages/server/src/routes/admin.ts` but live in the dashboard repo because the admin endpoints are intentionally not in the per-game `@junjo/sdk` surface (per Phase 10.2 + 11.2a + 11.3a decisions).
