...
- `routes/admin.ts` - cloud-only admin route handlers (Phase 10.2; marked `// @cloud-only`). Exports the `WireUserGames` / `WireUserGameRow` types and `listUserGamesHandler(prisma)` for `GET /v1/users/:junjoUserId/games`. The handler queries every `ExternalIdentity` row for the supplied internal `JunjoUser.id`, then runs a single batched `groupMember.findMany` to count active memberships in non-soft-deleted groups, tallying in memory (no N+1 counts). A `junjoUserId` with no identity rows returns `200` with `games: []` rather than 404 - the route cannot distinguish "user we have never seen" from "user known but no cross-game footprint yet" and collapses the cases. Games where the user has an identity but no active memberships are still listed with `joinedGroupCount: 0`. The route is registered in `app.ts` BEFORE the per-game `apiKeyMiddleware` (same pattern as the public `getInvitationByCodeHandler`), with a per-route `adminAuthMiddleware` so the admin token is the only auth check that runs.
