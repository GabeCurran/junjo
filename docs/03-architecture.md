...
- `routes/` - per-resource route modules. Each module exports a `<resource>Router(prisma)` factory that returns a fresh `Hono` sub-app, plus a sibling `<resource>.schema.ts` of co-located Zod schemas. `app.ts` wires them under the `/v1` namespace. Resource modules that own no router yet (only a wire-format builder + helpers shared by another router) live in the same directory: e.g., `routes/invitations.ts` exports `serializeInvitation` and `generateInvitationCode` for the `POST /v1/groups/:id/invitations` handler that lives inside `routes/groups.ts`. The standalone `/v1/invitations/:code` routes (Phase 2.3) will move into a sibling `invitationsRouter` factory in the same file when they land.
