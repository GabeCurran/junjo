...
- `identity.ts` - `findOrCreateJunjoUser(client, gameId, externalUserId)`. Resolves a dev's external user id (Clerk sub, Supabase uuid, Roblox UserId-as-string) to an internal `JunjoUser` id, creating both the `JunjoUser` and the `ExternalIdentity` row on first sight. Accepts either the full Prisma client or a transaction client so callers can fold the find-or-create into a multi-write transaction. Phase 2.4's accept-invitation flow is the first caller; Phase 6 (auth adapters) and Phase 10 (cross-game cloud) extend this pathway.
