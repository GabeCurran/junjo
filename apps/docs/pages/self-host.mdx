...
sequenceDiagram
  ...
  Route->>DB: WebhookEndpoint WHERE gameId AND disabledAt IS NULL<br/>AND (events IS EMPTY OR events contains type)
  ...
  loop every 5 seconds (setInterval, unref)
    ...
    loop each id, sequential
      ...
      Worker->>DB: SELECT WebhookDelivery + WebhookEndpoint
