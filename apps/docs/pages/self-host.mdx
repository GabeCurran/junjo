...
```sh
git clone https://github.com/gabe-curran/junjo
cd junjo
npm install
...
Validation lives in [`packages/server/src/env.ts`](https://github.com/gabe-curran/junjo/blob/main/packages/server/src/env.ts). Missing or malformed values throw a single readable error at startup; the process exits non-zero before binding the port.
