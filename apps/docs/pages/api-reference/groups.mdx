...
`createdBy` is `null` in V1 because no auth-adapter actor is wired through this code path. The field parallels `AuditEntry.actorUserId` and is reserved for a future iteration that threads the resolved end-user identity through invitation creation.
