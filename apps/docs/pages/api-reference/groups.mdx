...
| `roleId` | no | A role hint stored on the invitation. Not auto-applied at acceptance time today (call [`members.assignRole`](/sdk/members#assignrole) afterward) and not validated against the group's roles. |
