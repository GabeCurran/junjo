...
- The audit `payload` is `{ before, after }` carrying the prior and new parent ids (either may be null). The audit row's `targetId` is the new parent id (null when cleared). `actorUserId` is null in V1 (no auth-adapter actor wired).
