...
The diagram source lives at `tools/diagrams/source/webhook-delivery.mmd` and is kept byte-identical to this embedded fence (Phase 16.6 wires that as an automated gate; until then the rule is on the author).
...
- **No replay UI.** Manual replay of `failed` deliveries is not exposed through the V1 API; replays require direct database access. A future release will add a manual replay endpoint.
