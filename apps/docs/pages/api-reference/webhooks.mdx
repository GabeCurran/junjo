...
Receivers compute the same HMAC and compare in constant time. The SDK helper [`junjo.webhooks.verify`](../sdk/webhooks#verifyrawbody-headers-secret-opts) performs that check (and the [`junjo.webhooks.middleware`](../sdk/webhooks#middlewaresecret-opts) Express adapter wraps it); receivers in other languages can reproduce it with any HMAC-SHA256 library. Pseudocode:
...
Endpoints are configured per game through the `/v1/webhooks` routes below. The dev-facing SDK ergonomics live at [`junjo.webhooks.endpoints`](../sdk/webhooks#endpoints).

Each endpoint carries:

- `url` - destination for the POST. Must use `https://` or `http://`.
- `events` - array of event types this endpoint subscribes to. **Empty array means match-all** (the friendly default for "I want everything"). Non-empty array filters to the listed types; unknown event types are rejected at create / update time. Subscriptions accept every published event type - all 22 of them, including the seven `friend.*` types, `game.user.banned` / `game.user.unbanned`, and `member.banned` / `member.unbanned`. The canonical list is `WEBHOOK_EVENT_TYPES` in `packages/server/src/routes/webhooks.schema.ts`, kept in compile-time lockstep with the `JunjoEventType` union.
...
Returns `{ items: WebhookEndpoint[], nextCursor: string | null }` for the calling game, sorted by `createdAt desc, id desc`. The secret is not included. The response conforms to the same `Page<T>` envelope as every other list endpoint. Query parameters:

| Field | Type | Default | Notes |
|-------|------|---------|------|
| `limit` | integer (1-100) | 50 | Max endpoints per page. |
| `cursor` | string | none | The `nextCursor` from a previous page (the id of that page's last item). A cursor id that does not resolve inside the calling game returns `400 bad_request` (invalid cursor). |

`nextCursor` is the id of the last item when more endpoints exist; `null` on the final page. The SDK's `endpoints.listAll` walks the pages for you.
