...
| `status` | string | `"active"`, `"invited"`, `"left"`, `"kicked"`, or `"banned"`. |
...
| `bannedUntil` | string \| null | Only meaningful while a per-group ban is active (`status` is `"banned"`): `null` means a permanent ban, an ISO 8601 timestamp is when the ban lapses. A past timestamp means the ban has lapsed (lazy expiry; the server does not auto-flip `status`). Always `null` for non-banned members. |
