...
| code | status | when it is raised | how to handle |
|------|--------|-------------------|---------------|
| `invalid_config` | none | An auth adapter constructor (`jwtAdapter`, `clerkAdapter`, `supabaseAdapter`) was called with required arguments missing or of the wrong type. | Fix the adapter setup. The `message` field names the field at fault. |
| `not_implemented` | none | A method that is reserved in the SDK surface but not yet implemented (today: `Junjo#whoami`). | Do not call the method. The roadmap fills these in over time; new SDK versions remove the throw site. |
...
`status` is optional because SDK-only codes (`invalid_config`, `not_implemented`) do not have an HTTP status. For codes that originate on the server, `status` is always populated.
