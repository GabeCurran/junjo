...
- `viewer`: optional junjoUserId. When supplied, the visibility rules
  are enforced from that viewer's perspective; a viewer the rules do
  not permit (blocked by the target, or outside the owner's
  visibility setting) gets `404 not_found`, not an empty page, so a
  denied viewer cannot distinguish "hidden" from "no such user".
  Without it, the API-key caller is treated as admin (visibility
  bypassed).
