...
The hooks package depends on `@junjo.io/sdk` and React 18+. Both packages are pure ESM with TypeScript types bundled.
...
|--------|----------|-------|
| `apiKey` | yes, unless `proxy` | The full `prefix.secret` string from the dashboard or `db:seed`. Server-side only. |
