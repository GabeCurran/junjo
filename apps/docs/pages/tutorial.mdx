...
Junjo identifies users by your existing user id (whatever your auth provider returns). The tutorial uses string ids; your real code brands them as `UserId`.
...
For server-to-server delivery (your backend reacting to events from another process), use [webhooks](/api-reference/webhooks). Webhooks are HMAC-signed and retry with exponential backoff up to 6 attempts. The SDK provides `junjo.webhooks.verify` and `junjo.webhooks.middleware` for the receiver side.
...
- **[Webhooks](/api-reference/webhooks)** - dispatch events to your own backend, or pipe them to Discord and Slack.
- **[Auth adapters](/auth)** - so the user id you pass to Junjo is the verified user id from your auth provider.
- **[API reference](/api-reference)** - the raw HTTP surface, for languages Junjo does not yet ship a native client for.
