...
2. **`verify` / `middleware`** - receiver-side helpers that validate the HMAC signature on inbound deliveries and parse them into typed [`JunjoEvent`](./groups#subscribegroupid-handler-opts) values. Pair with the [HTTP wire format](../api-reference/webhooks) docs for the corresponding server side.
