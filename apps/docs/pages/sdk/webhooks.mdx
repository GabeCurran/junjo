...
type WebhookEndpointFormat = "junjo" | "discord";
```
...
`format` defaults to `"junjo"` (the raw `JunjoEvent` JSON with HMAC headers, the format `verify` and `middleware` consume above). Pass `"discord"` to translate events into [Discord embed payloads](../api/webhooks-discord) before POSTing - the URL is then a Discord webhook URL, the HMAC headers are skipped, and `verify` / `middleware` are not used (Discord receives the delivery directly).
