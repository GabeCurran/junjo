...
The methods listed as **shipped** are real implementations. `grantPermission` and `revokePermission` are stubbed and throw `not_implemented` until Phase 3.3.
...
| `permissions` | string[] | no | **Silently dropped in V1**: Phase 3.3 will populate role permissions via the grant / revoke routes. The field stays on `CreateRoleInput` for forward-compatibility, but the SDK strips it before sending. |
...
`grantPermission` and `revokePermission` are stubbed and throw `not_implemented` in V1. They land in Phase 3.3 of the roadmap (roles permissions surface).
