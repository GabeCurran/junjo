...
All methods on this page are shipped real implementations.

## `create(groupId, input)` - shipped
...
| `permissions` | string[] | no | **Silently dropped on create**: use [`grantPermission`](#grantpermissionroleid-permission---shipped) to add permission keys after the role exists. The field stays on `CreateRoleInput` so a typed object literal compiles either way; the SDK strips it before sending. |
...
Roles in V1 are a small list per group (10s, not 1000s); pagination is not provided. If the dev's role count grows past that, this method will gain pagination as an additive change.
...
## `grantPermission(roleId, permission)` - shipped
...
## `revokePermission(roleId, permission)` - shipped
