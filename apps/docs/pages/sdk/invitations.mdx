...
Methods on `junjo.invitations`. The full surface is defined; only methods marked **shipped** are wired to the server today. Calling a stubbed method throws `JunjoError` with `code: "not_implemented"`.
...
- [`GET /v1/groups/:id/invitations`](/api/groups) - the underlying HTTP route.
...
- [`GET /v1/invitations/:code`](/api/invitations) - the underlying HTTP route.
...
- [`DELETE /v1/invitations/:code`](/api/invitations) - the underlying HTTP route.
