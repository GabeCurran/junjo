...
Read the audit log for a group. The audit log is the durable record of every mutation Junjo has performed; the live event stream ([`junjo.groups.subscribe`](./groups#subscribegroupid-handler-opts---shipped)) is for transient UX, and `audit.list` is for "what has happened to this group, ever."
