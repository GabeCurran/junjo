...
| [`junjo.roles`](/sdk/roles) | `create`, `get`, `update`, `delete`, `list` shipped (Phase 3.1). `grantPermission` / `revokePermission` land in Phase 3.3. |
| [`junjo.members`](/sdk/members) | `get`, `getById`, `list`, `listForUser` shipped (Phase 2.6). Mutating methods land in Phase 2.7+. |
