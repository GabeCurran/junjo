...
`subscribe`, `setParent`, and `listChildren` still exist on the type surface today but throw `JunjoError("not_implemented")` until their phase ships. The shape is stable; they will fill in without breaking changes.
