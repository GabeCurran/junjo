...
| Field | Type | Default | Notes |
|-------|------|---------|-------|
| `roleId` | `RoleId` | none | A role hint stored on the invitation. Not auto-applied at acceptance time today (call [`junjo.members.assignRole`](/sdk/members#assignrole) afterward) and not validated against the group's roles. |
...
`Member` with `joinedAt: Date`. `roles` is `[]` immediately after redemption; the invitation's `roleId` is not auto-applied today, so call [`members.assignRole`](/sdk/members#assignrole) after acceptance if you need it.
...
For the inverse -- having a user join a group that allows public-join -- use `junjo.groups.join(groupId, userId, { passcode? })`. It returns the newly-active `Member` and honors any configured passcode gate; the route is rate-limited on per-IP and per-game incorrect attempts. Game and group ban checks both run before the join succeeds (a banned user receives `403 banned`).
