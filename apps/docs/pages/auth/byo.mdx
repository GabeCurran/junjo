...
The `RobloxUserIdAdapter` shipped by [`junjo-roblox`](/roblox) (the Luau SDK) is an equivalent adapter for use inside Roblox Studio. The recipe above is the TypeScript counterpart for the non-Roblox half of the integration (a Node backend that the Roblox script forwards requests to).
