...
sequenceDiagram
  ...
  Server->>DB: ExternalIdentity (gameId, externalUserId)
  alt mapping exists
    ...
    alt insert wins (no concurrent first-appearance)
      ...
      Server->>DB: ExternalIdentity (gameId, externalUserId) re-select
...
The diagram above is committed at `tools/diagrams/source/auth-flow.mmd`. The committed `.mmd` source and the Mermaid fence above must stay byte-identical (Phase 16.6 wires a sync gate).
