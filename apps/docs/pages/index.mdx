...
- **[SDK](/sdk)** - per-namespace reference for `@junjo.io/sdk` (`junjo.groups`, `junjo.members`, `junjo.roles`, `junjo.invitations`, `junjo.audit`, `junjo.webhooks`, `junjo.friends`, `junjo.bans`, plus the top-level `can` / `check`).
- **[React](/react)** - hooks and provider in `@junjo.io/react`.
- **[Roblox](/roblox)** - the Luau SDK for Roblox game servers.
- **[C++](/cpp)** - the C++20 SDK for game servers and backends.
- **[Unreal Engine](/unreal)** - the source plugin over the C++ core: subsystem, Blueprint async nodes, live event streams.
- **[Feature matrix](/feature-matrix)** - what each of the five SDKs covers, operation by operation.
- **[API](/api-reference)** - the HTTP API the SDKs wrap. Use raw HTTP from any language outside the TypeScript, React, Roblox, C++, and Unreal Engine set.
