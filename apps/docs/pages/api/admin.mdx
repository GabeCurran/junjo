...
Not part of the per-game `@junjo/sdk` surface. The dashboard calls this endpoint directly via `fetch` because the per-game SDK shape (one `Junjo` instance, one API key, one game) does not fit a server-wide query.
