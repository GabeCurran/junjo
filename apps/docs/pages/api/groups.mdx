...
Creates a direct-user invitation. The recipient is the dev's external user id (the one returned by the configured auth adapter). The server picks a unique 16-character hex code; future endpoints (`GET /v1/invitations/:code`, accept/decline) read invitations by that code.
...
|-------|----------|-------|
| `targetUserId` | yes | The external user id from the dev's auth provider. Stored verbatim; the server does not require a corresponding `JunjoUser` row at invite time. |
...
A missing `targetUserId`, an empty string, or a malformed JSON body all return `400 bad_request`.
...
| `targetId` | the invited `targetUserId` |
| `actorUserId` | `null` |
| `payload` | `{ invitationId, code, targetUserId, roleId }` |

### Errors

| Code | Status | When |
|------|--------|------|
| `bad_request` | 400 | Body fails validation, malformed JSON. |
