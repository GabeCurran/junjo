...
- `secret` - the HMAC signing key. Stored in recoverable form so the worker can sign requests; surfaced exactly once on `POST /v1/webhooks` and never again. Persist it on creation.
...
Body: `{ url, events?, secret? }`. If `secret` is omitted the server generates a 32-byte base64url secret. The response includes `secret` exactly once - persist it server-side immediately. `events` defaults to `[]` (match-all).
...
Body: `{ url?, events?, disabled? }`. At least one field is required.
...
Returns the post-state `WebhookEndpoint` (no secret). A no-op PATCH (matching url, events, disabled) returns the unchanged row without writing.
