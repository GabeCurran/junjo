...
The `actions` strings must match the [`AuditAction`](../../sdk/index) union in `@junjo/shared`. Unknown values are rejected with `400 bad_request`.
...
| `actorUserId` | string \| null | The Junjo user id that performed the action, when known. `null` for system-driven actions and for actions taken via the API key (no auth-adapter actor wired in V1 for most mutations). |
