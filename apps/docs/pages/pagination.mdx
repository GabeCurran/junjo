...
**Roles are the one shape exception.** `GET /v1/groups/:id/roles` answers with a bare `Role[]` by default, not a `{ items }` envelope, because a published client already deserializes it as an array. Pass `?paged=true` for the envelope (`nextCursor` is always `null`; the route returns every role). In the TypeScript SDK you need neither: `roles.list()` returns a value that is both an array and a page, so array methods and `.items` / `.nextCursor` both work. See [Roles](/api-reference/roles#the-two-response-shapes).
