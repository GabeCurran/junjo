...
The live hooks in the group family ([`useGroup`](/react/use-group), [`useMembers`](/react/use-members), [`useInvitations`](/react/use-invitations), [`useRoles`](/react/use-roles)) do not each open their own SSE connection. The provider owns a subscription hub that keeps at most one stream per `(client, groupId)` pair:
