...
The cache has no TTL and no event-driven invalidation in V1. The server's `permissions/check` endpoint runs its own 60-second per-process cache that invalidates on role / permission mutations, but that invalidation does not propagate to the React cache. If a permission can flip mid-session, you currently need to remount the consuming component or wait for the next provider mount. A reactive invalidation layer is on the Phase 7.5 roadmap and will be transparent when it lands.
