...
After `applyOptimistic` runs, the hook keeps applying SSE events (`member.invited`, `member.joined`) on top of the optimistic state. A successful `revoke` does not emit an event in V1, so the optimistic removal stays applied indefinitely; if the mutation fails and you roll back, the row reappears in the list, ready to retry.
...
Each Phase 7 list hook opens its own SSE subscription. If a screen renders both `useInvitations` and `useGroup` (or `useMembers`) for the same `groupId`, two or three parallel streams open against the server. This is correct in V1 (the server tolerates duplicate subscriptions per group) but wasteful for multi-consumer pages. Phase 7.5 will introduce a shared subscription cache that deduplicates streams under one provider.
