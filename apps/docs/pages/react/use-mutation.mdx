...
When you are mutating the roster from `useMembers`, prefer its `applyOptimistic` helper - it owns the snapshot lifecycle for you and returns a rollback closure:
...
For state that does not have a hook-level helper (e.g., `useGroup` and `useInvitations` until Phase 7.5c), the explicit snapshot pattern still applies: hold the prior value in `useState` (or a ref), apply the optimistic mutation in `onMutate`, return the snapshot as the context, and restore it from `onError`.
