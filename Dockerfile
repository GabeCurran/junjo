...
RUN if [ "$WORKSPACE" = "server" ]; then \
      npm ci --workspace @junjo/shared --workspace @junjo/server --include-workspace-root --include=dev; \
    elif [ "$WORKSPACE" = "dashboard" ]; then \
      npm ci --workspace @junjo/shared --workspace @junjo/sdk --workspace @junjo/dashboard --include-workspace-root --include=dev; \
...
RUN if [ "$WORKSPACE" = "server" ]; then \
      npm run build -w @junjo/shared && npm run build -w @junjo/server; \
    elif [ "$WORKSPACE" = "dashboard" ]; then \
      npm run build -w @junjo/shared && npm run build -w @junjo/sdk && npm run build -w @junjo/dashboard; \
    elif [ "$WORKSPACE" = "docs" ]; then \
