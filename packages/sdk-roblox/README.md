...
## What ships today (Phase 8.1 + 8.2)

The `Junjo.new(config)` factory, the HTTP wrapper, and per-namespace methods that mirror the TypeScript SDK: `groups`, `members`, `roles`, `invitations`, `audit`, `webhooks.endpoints`, plus the top-level `junjo:can(...)` / `junjo:check(...)` permission helpers.
...
local invitation = junjo.groups:inviteByUserId(group.id, tostring(player.UserId))

local allowed = junjo:can(tostring(player.UserId), group.id, "invite_member")
```
...
Dogfood target: the `mobarena-roblox` project. Source lives in `src/init.lua` plus per-namespace siblings (`groups.lua`, `members.lua`, etc.). Full reference at [`apps/docs/pages/roblox/index.mdx`](../../apps/docs/pages/roblox/index.mdx).
